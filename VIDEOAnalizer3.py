...
def crear_plantilla_email_tangenciales_html(cliente, items_list, hora_ciclo):
    """
    ...
    for i, tang in enumerate(items_list, 1):
        ...
        mm = int(ts_seg // 60)
        ss = int(ts_seg % 60)
        tiempo = f"{mm}m{ss:02d}s"
        arch = html_module.escape(str(tang.get('archivo', '')))
        term = html_module.escape(str(tang.get('termino', '')))
        motivo = html_module.escape(str(tang.get('motivo', '')))
        ...
        filas.append(f"""
            <tr style="background:{bg};">
                ...
                <td style="padding:14px 12px;border-bottom:1px solid #e9ecef;font-size:13px;word-break:break-word;">{arch}</td>
                ...
                <td style="padding:14px 12px;border-bottom:1px solid #e9ecef;white-space:nowrap;font-family:Consolas,monospace;">{html_module.escape(tiempo)}</td>
            </tr>""")
    ...
    <div style="max-width:820px;margin:0 auto;padding:24px;">
        <div style="background:#fff;border-radius:16px;box-shadow:0 8px 32px rgba(0,0,0,0.08);overflow:hidden;">
            <div style="background:linear-gradient(135deg, {primary} 0%, {secondary} 100%);color:#fff;padding:28px 32px;text-align:center;">
                ...
                <p style="margin:8px 0 0 0;font-size:14px;opacity:0.88;">Cierre de ciclo: {hora_safe}</p>
            </div>
            <div style="padding:28px 24px 8px 24px;">
                <p style="margin:0 0 20px 0;color:#6c757d;font-size:14px;line-height:1.6;">
                    A continuación, los segmentos <strong>no enviados como alerta con clip</strong>: descartes por relevancia baja o mención tangencial sin desarrollo suficiente.
                </p>
                    ...
                    <table style="width:100%;border-collapse:collapse;font-size:14px;">
                        <thead>
                            <tr style="background:linear-gradient(180deg,#f8f9fa 0%,#eef1f5 100%);">
                                ...
                                <th style="padding:12px;text-align:left;border-bottom:2px solid {primary};color:#343a40;">Archivo</th>
                                ...
                                <th style="padding:12px;text-align:left;border-bottom:2px solid {primary};color:#343a40;">Tiempo</th>
                            </tr>
                        </thead>
                        <tbody>
                            {tbody}
...
def enviar_brevo_menciones_tangenciales_cliente(cliente, items_list, hora_ciclo=None):
    ...
    for i, tang in enumerate(items_list, 1):
        ...
        ss = int(ts_seg % 60)
        lineas_txt.append(
            f"{i}. archivo: {tang.get('archivo', '')} | término: {tang.get('termino', '')} | "
            f"motivo: {tang.get('motivo', '')} | tiempo: {mm}m{ss:02d}s"
        )
...
def enviar_coincidencia_inmediata(nombre_archivo, termino_encontrado, contexto_termino, tipo_archivo, clip_path=None, transcripcion_completa="", timestamp=None, idea_general=None, video_url=None, transcripcion_segmento=""):
    ...
    try:
        ...
        if isinstance(resultados, dict) and resultados.get("_dedupe_skip"):
            ...
            return True, razon, video_url_cloudinary or video_url
        ...
        return True, f"Enviado a cliente: {cliente_nombre}", video_url_cloudinary or video_url
...
def append_analisishoy_menciones_tangenciales(menciones_tangenciales_data):
    ...
    try:
        ...
        for i, tangencial in enumerate(menciones_tangenciales_data, 1):
            ...
            mm = int(ts_seg // 60)
            ss = int(ts_seg % 60)
            arch = tangencial.get('archivo', '')
            term = tangencial.get('termino', '')
            motivo = tangencial.get('motivo', '')
            lineas.append(
                f"- **{i}.** `{arch}` | termino: **{term}** | "
                f"motivo: {motivo} | tiempo: {mm}m{ss:02d}s"
            )
        lineas.append("")
        bloque = "\n".join(lineas)
    ...
    try:
        ...
        if use_c:
            ...
            if video_url_cloudinary:
                ...
                log_info(
                    f"Cloudinary falló ({upload_msg}); reintentando envío directo sendVideo ({file_size_mb:.1f}MB)",
                ...
                if encontrado:
                    ...
                    try:
                        ...
                        if segmento_gemini is None:
                            ...
                            menciones_tangenciales_data.append({
                                'archivo': rel,
                                'termino': termino,
                                'motivo': 'Mencion tangencial sin desarrollo',
                            ...
                            if not es_relevante_gemini or relevancia_gemini == 'baja':
                                if es_prioritario:
                                    ...
                                    menciones_tangenciales_data.append({
                                        'archivo': rel,
                                        'termino': termino,
                                        'motivo': f"Relevancia baja ({relevancia_gemini})",
                                        'timestamp': momento_termino
                                    })
                                    _destino_video_check, _txt_video_check, estado_videoscheck = guardar_video_y_minuto_coincidencia(
                                        video_path=archivo_path,
    ...
    if videos_procesados_data or menciones_tangenciales_data:
        ...
        if menciones_tangenciales_data:
            ...
            for i, tangencial in enumerate(menciones_tangenciales_data, 1):
                ...
                ss = int(ts_seg % 60)
                st.markdown(
                    f"- **{i}.** `{tangencial.get('archivo', '')}` | termino: **{tangencial.get('termino', '')}** | "
                    f"motivo: {tangencial.get('motivo', '')} | tiempo: {mm}m{ss:02d}s"
                )
